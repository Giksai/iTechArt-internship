...
const selectors = {
    ...
    authSelectionElement: (authType) => {
        return `//nav[@class="auth__menu"]/a[text()="${authType}"]`;
    },
    searchingParameters: (parameter) => {
        return `//select[@name="searchFor"]//option[@value="${parameter}"]`;
    },
    productsCatalogItem: (element) => {
        return `//div[@class="menu__cat-drop"]/a[span="${element}"]`;
    },
